...
export default ({ type }) => {
    ...
    // https://github.com/stream-protocol/wallet/pull/2178
    ...
    if (type === 'authorized-apps') {
        return ( <
            AuthorizedAppsKeys authorizedAppsKeys = { authorizedAppsKeys }
            onClickDeAuthorizeKey = {
                (publicKey) => deAuthorizeKey(publicKey)
            }
    ...
    if (type === 'full-access-keys') {
        return ( <
            FullAccessKeys fullAccessKeys = { fullAccessKeys }
            onClickDeAuthorizeKey = {
                (publicKey) => deAuthorizeKey(publicKey)
            }
            userInputAccountId = { userInputAccountId }
            setUserInputAccountId = {
                (userInputAccountId) => setUserInputAccountId(userInputAccountId)
            }
            accountId = { accountId }
            confirmDeAuthorizeKey = { confirmDeAuthorizeKey }
            setConfirmDeAuthorizeKey = {
                (publicKey) => setConfirmDeAuthorizeKey(publicKey)
            }
            deAuthorizingKey = { deAuthorizingKey }
            />
        );
