...
export function EnableTwoFactor(props) {
    ...
    if (!initiated) {
        return ( <
            ...
            <
            AlertBanner title = 'twoFactor.alertBanner.title'
            data = { getNearAndFiatValue(multiSigMinAmountRaw, nearTokenFiatValueUSD) }
            button = 'twoFactor.alertBanner.button'
            theme = 'alert'
            linkTo = 'https://docs.streamprotocol.app/docs/concepts/storage-staking' /
            >
            <
            form onSubmit = {
                (e) => {
            ...
            h4 > < Translate id = 'twoFactor.select' / > < span > * < /span></h
            4 >
            <
            TwoFactorOption onClick = {
                () => setOption('email')
            }
            option = 'email'
            active = { option } >
            ...
            label >
            <
            Checkbox checked = { twoFactorAmountApproved }
            onChange = {
                (e) => setTwoFactorAmountApproved(e.target.checked)
            }
            /> <
            span > < SafeTranslate id = 'twoFactor.checkBox'
            data = {
                { amount: getNearAndFiatValue(multiSigMinAmountRaw, nearTokenFiatValueUSD) }
            }
            /></span >
            <
            /label> <
        ...
        return ( <
            EnterVerificationCode option = { option }
            email = { email }
