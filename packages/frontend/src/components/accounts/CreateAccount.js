...
import DepositStreamBanner from '../wallet/DepositStreamBanner';
    ...
    render() {
        ...
        if (showTermsPage) {
            return ( <
                ...
                h2 >
                <
                SafeTranslate id = 'createAccount.termsPage.descOne'
                data = {
                    { amount: getNearAndFiatValue(MIN_BALANCE_TO_CREATE, nearTokenFiatValueUSD) } }
                /> <
                /h2> <
                ...
                /div> {
                    whereToBuy && ( <
                        WhereToBuyNearModal onClose = {
                            () => this.setState({ whereToBuy: false }) }
                        open = { whereToBuy }
                        />
                    )
                } <
        ...
        if (!invalidNearDrop) {
            return ( <
                ...
                AccountFormAccountId mainLoader = { mainLoader }
                ...
                /> <
                AccountNote / > { cannotCreateNewAccountWithZeroBalanceAccount && < DepositStreamBanner / > } <
