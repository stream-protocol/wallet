...
export default ({
    ...
    return ( <
        ...
        FormButton linkTo = { buildUtorgPayLink(implicitAccountId) }
        color = 'black'
        sending = {!implicitAccountId }
        sendingString = 'button.loading' >
        <
        Translate id = 'buyStrm.buyWith' / >
        <
        img src = { UtorgLogo }
        alt = 'utorg' / >
        <
        /FormButton> <
        FormButton disabled = {!moonpayIsAvailable }
        ...
        linkTo = { moonpaySignedUrl } > {
            moonpayIsAvailable ? ( <
                ...
                <
                Translate id = 'buyStrm.notSupported' / >
                <
                />
            )
        } <
        /FormButton> <
        h3 className = 'bottom' > < Translate id = 'account.createImplicitAccount.orSendNear'
        data = {
            { amount: formattedMinDeposit }
        }
        /></h
        3 >
        <
        Translate id = 'account.createImplicitAccount.sendFrom' / > & nbsp; <
        FormButton onClick = {
            () => setShowWhereToBuyModal(true)
        }
        className = 'link underline' > < Translate id = 'account.createImplicitAccount.exchange' / > < /FormButton>,<br / >
        <
