...
test('formatStrmAmount when given 0', () => {
    expect(formatStrmAmount(contextZero)).toBe('0');
});

test('formatStrmAmount when given new BN(0)', () => {
    expect(formatNearAmount(contextZeroBN)).toBe('0');
});

test('formatStrmAmount when given tiny raw amount', () => {
    expect(formatNearAmount(contextTiny)).toBe('< 0.00001');
});

test('formatStrmAmount when given small raw amount', () => {
    expect(formatNearAmount(contextSmall)).toBe('0.00001');
});

test('formatStrmAmount when given large raw amount', () => {
    expect(formatStrmAmount(contextBig)).toBe('1,234.567');
});

/* showInYocto */

test('showInYocto when given 0', () => {
    expect(showInYocto(contextZero)).toBe('0 yoctoSTRM');
});

test('showInYocto when given new BN(0)', () => {
    expect(showInYocto(contextZeroBN)).toBe('0 yoctoSTRM');
});

test('showInYocto when given tiny raw amount', () => {
    expect(showInYocto(contextTiny)).toBe('123,456 yoctoSTRM');
});

test('showInYocto when given small raw amount', () => {
    expect(showInYocto(contextSmall)).toBe('10,000,000,000,000,000,000 yoctoSTRM');
});

test('showInYocto when given large raw amount', () => {
    expect(showInYocto(contextBig)).toBe('1,234,567,000,000,000,000,000,000,000 yoctoSTRM');
});
